...
from hejasverige.ims.interfaces import IIMSMessage, ISentMessage, IReceivedMessage, ISendable
...
class IMSMessage(object):
    ...
    """ Adapter to send Heja Sverige IMS messages
    """
    ...
    def _getOwner(self, container):
        try:
            user = container.getWrappedOwner()
                                ...
                                # , e:  we have a zope rather than a plone user
            owner = [u for (u, roles) in container.get_local_roles() if 'Owner'
                      in roles][0]
